...
const cloudshellScriptPath = join(root, 'outputs', 'polly-cloudshell-generate.sh');
const fps = 30;
const pauseFrames = 8;
const scenePadFrames = 28;
const minSceneFrames = 150;
const voiceRate = '245';
const provider = process.env.VOICEOVER_PROVIDER || 'mac';
const pollyEngine = process.env.POLLY_ENGINE || 'standard';
const pollyRegion = process.env.POLLY_REGION || 'us-east-2';
const pollySourceDir = process.env.POLLY_SOURCE_DIR || join(root, 'outputs', 'polly-voiceover', 'voiceover');
...
mkdirSync(dirname(cloudshellScriptPath), {recursive: true});

const cloudShellLines = [
  '#!/usr/bin/env bash',
  'set -euo pipefail',
...
scenes.forEach((scene, sceneIndex) => {
  ...
  scene.lines.forEach((line, lineIndex) => {
    ...
    const pollyPath = `voiceover/${stem}.mp3`;

    if (provider === 'polly-script') {
      cloudShellLines.push(
        [
          'aws polly synthesize-speech',
          `--region ${shQuote(pollyRegion)}`,
          `--engine ${shQuote(pollyEngine)}`,
          `--voice-id ${shQuote(voices[line.speaker].polly)}`,
          '--output-format mp3',
          `--text ${shQuote(line.text)}`,
          shQuote(pollyPath),
        ].join(' ')
      );
    } else if (provider === 'polly') {
      copyFileSync(join(pollySourceDir, `${stem}.mp3`), mp3);
    } else {
      shell('say', ['-v', voices[line.speaker].mac, '-r', voiceRate, '-o', aiff, line.text]);
      shell('ffmpeg', ['-y', '-v', 'error', '-i', aiff, '-codec:a', 'libmp3lame', '-q:a', '4', mp3]);
      rmSync(aiff, {force: true});
    }

    if (provider === 'polly-script') {
      localFrame += 180 + pauseFrames;
      globalLineIndex += 1;
      return;
    }
...
if (provider === 'polly-script') {
  cloudShellLines.push('tar -cf webbyelie-polly-voiceover.tar voiceover');
  cloudShellLines.push('ls -lh webbyelie-polly-voiceover.tar');
  writeFileSync(cloudshellScriptPath, `${cloudShellLines.join('\n')}\n`);
  console.log(`Wrote ${cloudshellScriptPath}`);
  console.log('Upload/paste this script into AWS CloudShell, then download webbyelie-polly-voiceover.tar.');
  process.exit(0);
}
