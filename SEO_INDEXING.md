...
Current public version: `v248.1` (preview branch; Cloudflare production remains on its prior release)
...
Bing flagged one homepage SEO/GEO notice for the logo image alt text. The visible brand mark now has a meaningful `alt`, and that fix remains present in `v246.2`.
...
- Re-run Bing URL Inspection for the homepage after `v246.2` is deployed and crawled to confirm the logo alt notice clears.
